...
public:
    ...
    void step(float dt = 1.f) {
        ...
        for(size_t i=0;i<ants.size();++i) {
            Ant &a=ants[i];
            updateAnt(a,dt);
            if(a.energy<=0.f || a.age>a.lifespan) { a=offspring(); ++deaths; ++births; }
        }
    ...
    bool save(const std::string& path) const {
        ...
        const uint32_t magic=0x41564F31, version=2; f.write((char*)&magic,4); f.write((char*)&version,4);
    ...
    bool load(const std::string& path) {
        ...
        if(magic!=0x41564F31||v!=2) return false;
...
private:
    ...
    Genes randomGenes(){ return {rnd(.65f,1.35f),rnd(9,28),rnd(.25f,.9f),rnd(.08f,.32f),rnd(.75f,1.25f),rnd(.7f,1.4f),rnd(.04f,.25f)}; }
    Ant randomAnt(){
        float a=rnd(-PI,PI),r=rnd(0,34); uint32_t life=uint32_t(rnd(14000,26000));
        uint32_t startingAge=uint32_t(rnd(0,float(life)*.55f));
        return{nestX+cosf(a)*r,nestY+sinf(a)*r,a+rnd(-.55f,.55f),rnd(650,1100),startingAge,life,0,randomGenes(),false};
    }
    ...
    Genes mutate(const Genes&a,const Genes&b){
        ...
        return {m(a.speed,b.speed,.45f,1.8f),m(a.sensorDistance,b.sensorDistance,6,42),m(a.sensorAngle,b.sensorAngle,.12f,1.25f),
                m(a.turnRate,b.turnRate,.04f,.55f),m(a.metabolism,b.metabolism,.5f,1.7f),m(a.deposit,b.deposit,.35f,2.2f),m(a.exploration,b.exploration,.01f,.45f)};
    }
    const Ant& parent(){
    ...
    Ant offspring(){
        const Ant&p1=parent(),&p2=parent();float a=rnd(-PI,PI),r=rnd(8,42);float ration=std::clamp(colonyFood,0.f,80.f);colonyFood-=ration;
        uint32_t life=uint32_t(rnd(14000,26000));
        return{nestX+cosf(a)*r,nestY+sinf(a)*r,a+rnd(-.35f,.35f),720.f+ration*3.f,0,life,0,mutate(p1.genes,p2.genes),false};
    }
    ...
    void updateAnt(Ant&a,float dt){
        ...
        float l=sense(-sa),c=sense(0),r=sense(sa), strongest=std::max({l,c,r});
        // Follow a meaningful gradient. Equal sensors choose randomly instead of
        // always turning left, which previously created stable circular loops.
        if(strongest>.12f && (c<l||c<r)){
            float difference=r-l;
            float direction=std::fabs(difference)<.025f?(unit(rng)<.5f?-1.f:1.f):(difference>0?1.f:-1.f);
            a.angle+=direction*a.genes.turnRate*dt;
        }
        // Weak or absent trails mean free exploration; established trails reduce noise.
        // Small continuous noise creates long searching runs; too much noise
        // makes a random walker scribble around one spot instead of exploring.
        float wander=strongest<.12f?.48f:(strongest<1.f?.32f:.18f);
        a.angle+=rnd(-1,1)*a.genes.exploration*wander*dt;
        if(unit(rng)<.0010f*dt) a.angle+=rnd(-1.15f,1.15f); // occasional loop-breaking turn
        if(!a.carrying){
            float nx=a.x-nestX,ny=a.y-nestY,dist2=nx*nx+ny*ny;
            if(dist2<125.f*125.f && dist2>1.f){
                float outward=atan2f(ny,nx),d=atan2f(sinf(outward-a.angle),cosf(outward-a.angle));
                a.angle+=d*.09f*dt; // disperse before selecting a trail
            }
        }
        if(a.carrying){
            float target=atan2f(nestY-a.y,nestX-a.x),d=atan2f(sinf(target-a.angle),cosf(target-a.angle));
            a.angle+=d*.16f*dt; // a loaded ant prioritizes getting home over trail noise
        }
